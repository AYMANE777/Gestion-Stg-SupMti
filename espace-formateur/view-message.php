...
if($message['receiver_id'] == $matricule || $message['sender_id'] == $matricule)
{
    if($message['receiver_id'] == $matricule)
	{
		...
		$QueryGetSender = mysqli_query($connect, "SELECT * FROM stagiaires WHERE matricule='$sender_id'");
		$stagiaire = mysqli_fetch_assoc($QueryGetSender);
		$sender_name = $stagiaire['prenom'] . " " . $stagiaire['nom'];
		$receiver_name = "moi";
	}else if($message['sender_id'] == $matricule)
	{
		$receiver_id = $message['receiver_id'];
		$QueryGetSender = mysqli_query($connect, "SELECT * FROM stagiaires WHERE matricule='$receiver_id'");
		$stagiaire = mysqli_fetch_assoc($QueryGetSender);
		$sender_name = $user->getPrenom() . " ". $user->getNom();
		$receiver_name = $stagiaire['prenom'] . " " . $stagiaire['nom'];
	}
}else{
	echo '<script>location.replace("mes-messages")</script>';
}
			...
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="d-flex flex-wrap gap-2 justify-content-between mb-8">
						...
						<div class="d-flex">
							<!--begin::Print-->
							<a href="#" onclick="window.print()" class="btn btn-sm btn-icon btn-light btn-active-light-primary me-2" data-bs-toggle="tooltip" data-bs-placement="top" title="" data-bs-original-title="Print">
						...
						if(mysqli_num_rows($QueryGetReplies) > 0)
						{
							while($reply = mysqli_fetch_assoc($QueryGetReplies))
							{
								if($reply['sender_id'] == $matricule)
								{
									...
									$QueryGetReplier = mysqli_query($connect, "SELECT * FROM stagiaires WHERE matricule='$reply_sender_id'");
									$stagiaire = mysqli_fetch_assoc($QueryGetReplier);
									$reply_sender = $stagiaire['prenom'] . " " . $stagiaire['nom'];
								}
...
<input type="hidden" id="currentPage" value="<?php echo $currentFileName; ?>">
<?php
