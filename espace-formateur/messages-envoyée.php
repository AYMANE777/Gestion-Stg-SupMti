...
$QueryGetMessages = mysqli_query($connect, "SELECT message_id, subject, content, date_sent, isRead
FROM messages
WHERE sender_id=$mat AND isDeleted=0 ORDER BY message_id DESC");
					...
					<div class="card">
						<div class="card-header align-items-center py-5 gap-2 gap-md-5">
							...
							<div class="d-flex flex-wrap gap-1">
								...
								<!--end::Reload-->
								<!--begin::Delete-->
								<button type="button" id="deleteMessages" class="btn btn-sm btn-icon btn-light btn-active-light-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Supprimer">
                                            ...
                                            while($message = mysqli_fetch_assoc($QueryGetMessages))
                                            {
                                                ...
                                                <td class="ps-9">
                                                    <div class="form-check form-check-sm form-check-custom form-check-solid mt-3">
                                                        <input class="form-check-input" type="checkbox" data-checkbox="message-inbox" value="'.$message['message_id'].'" />
                                                    </div>
                                                </td>
                                                    ...
                                                    <a href="#" class="d-flex align-items-center text-dark">
                                                        <div class="symbol symbol-35px me-3">
                                                            <div class="symbol-label bg-light-danger">
                                                                <span class="text-danger">'.substr($user->getPrenom(), 0, 1).'</span>
                                                            </div>
                                                        </div>
                                                        <span class="fw-bold '.$text.'">'.$user->getPrenom().' '.$user->getNom().'</span>
                                                    </a>
                                                </td>
                                                <td>
                                                    <div class="text-dark mb-1">
                                                        <a href="view-message?message-id='.$message['message_id'].'" class="text-dark">
                                                            <span class="fw-bolder '.$text.'">'.$message['subject'].'</span>
                                                        </a>
                                                    </div>
                                                    <div class="badge badge-light-warning">Envoyée</div>
                                                </td>
